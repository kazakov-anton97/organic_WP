...
<body <?php body_class();?>>
	...
	<div class="body_wrap<?php echo !empty($class) ? ' '.esc_attr($class) : ''; ?>"<?php echo !empty($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
		...
		<div class="page_wrap">
			...
			$show_title = organic_beauty_get_custom_option('show_page_title')=='yes' && !is_home() && !is_front_page();
			$show_navi = apply_filters('organic_beauty_filter_show_post_navi', false);
			$show_breadcrumbs = organic_beauty_get_custom_option('show_breadcrumbs')=='yes' && !is_home() && !is_front_page();
						...
						<div class="content_wrap">
							...
							if ($show_title) {
								if ($show_navi) {
									...
									?><h1 class="page_title"><?php echo wp_kses_post(organic_beauty_get_blog_title()); ?></h1><?php
								}
							}
							if ($show_breadcrumbs) {
