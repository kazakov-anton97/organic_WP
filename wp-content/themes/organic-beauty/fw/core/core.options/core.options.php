...
if ( !function_exists( 'organic_beauty_options_admin_menu_item' ) ) {
	//Handler of add_action('admin_menu', 'organic_beauty_options_admin_menu_item');
...
if ( !function_exists( 'organic_beauty_get_custom_option' ) ) {
	function organic_beauty_get_custom_option($name, $defa=null, $post_id=0, $post_type='post', $tax_id=0, $tax_type='category') {
		if (isset($_GET[$name]))
			$rez = organic_beauty_get_value_gp($name);
		else {
...
if ( !function_exists( 'organic_beauty_is_inherit_option' ) ) {
	function organic_beauty_is_inherit_option($value) {
		...
		return organic_beauty_strtolower($value)=='inherit';
	}
}
...
if ( !function_exists( 'organic_beauty_options_load_scripts' ) ) {
	function organic_beauty_options_load_scripts() {
		...
		if (!$loaded) {
			...
			wp_enqueue_style( 'fontello-admin-style',	organic_beauty_get_file_url('css/fontello-admin/css/fontello-admin.css'), array(), null);
			wp_enqueue_style( 'fontello-style', 			organic_beauty_get_file_url('css/fontello/css/fontello.css'), array(), null);
			wp_enqueue_style( 'fontello-animation-style',organic_beauty_get_file_url('css/fontello-admin/css/animation.css'), array(), null);
			// Organic Beauty options styles
			wp_enqueue_style('organic-beauty-options-style',			organic_beauty_get_file_url('core/core.options/css/core.options.css'), array(), null);
			wp_enqueue_style('organic-beauty-options-datepicker-style',	organic_beauty_get_file_url('core/core.options/css/core.options-datepicker.css'), array(), null);
			...
			wp_enqueue_script('colors-script',		organic_beauty_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );
			wp_enqueue_script('colorpicker-script',	organic_beauty_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );

			// Input masks for text fields
			wp_enqueue_script( 'jquery-input-mask',				organic_beauty_get_file_url('core/core.options/js/jquery.maskedinput.min.js'), array('jquery'), null, true );
			// Organic Beauty core scripts
			wp_enqueue_script( 'organic-beauty-core-utils-script',		organic_beauty_get_file_url('js/core.utils.js'), array(), null, true );
			// Organic Beauty options scripts
			wp_enqueue_script( 'organic-beauty-options-script',			organic_beauty_get_file_url('core/core.options/js/core.options.js'), array('jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-accordion', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-datepicker'), null, true );
			wp_enqueue_script( 'organic-beauty-options-custom-script',	organic_beauty_get_file_url('core/core.options/js/core.options-custom.js'), array('organic_beauty-options-script'), null, true );
		
			wp_localize_script( 'organic-beauty-options-script', 'ORGANIC_BEAUTY_OPTIONS_DATA', organic_beauty_storage_get('to_data') );
...
if ( !function_exists( 'organic_beauty_options_page' ) ) {
	function organic_beauty_options_page() {
		...
		$page = isset($_REQUEST['page']) ? organic_beauty_get_value_gp('page') : '';
		$mode = isset($_REQUEST['subpage']) ? organic_beauty_get_value_gp('subpage') : '';
		$override = $slug = '';
		if (!empty($mode)) {
			$inheritance = organic_beauty_get_theme_inheritance();
			if (!empty($inheritance) && is_array($inheritance)) {
			...
			if ($field['type'] != 'hidden') {
				...
				if ( !in_array($field['type'], array('divider'))) {
					echo '<label class="organic_beauty_options_field_label'
						...
						. '>' 
						. ($field['title']) 
						. (!empty($field['info']) && ($fdir=organic_beauty_get_file_url('images/to_info/'.sanitize_file_name($id).'.jpg'))!=''
							? '<a href="'.esc_url($fdir).'" data-rel="popup" target="_blank" class="organic_beauty_options_field_label_info iconadmin-info-circled" title="'.esc_attr__('More info', 'organic-beauty').'"></a>'
							: '')
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			case 'textarea':
				...
				echo '<textarea class="organic_beauty_options_input organic_beauty_options_input_textarea"'
					...
					. (!empty($field['action']) ? ' onchange="organic_beauty_options_action_'.esc_attr($field['action']).'(this);return false;"' : '')
					. '>'
					. esc_html(organic_beauty_is_inherit_option($clone_val) ? '' : $clone_val)
					. '</textarea>';
			break;
				...
				if (!empty($clone_val) && !organic_beauty_is_inherit_option($clone_val)) {
					...
					echo '<a class="organic_beauty_options_image_preview" data-rel="popup" target="_blank" href="'.esc_url($clone_val).'">'
							. (!empty($ext) && organic_beauty_strpos('jpg,png,gif', $ext)!==false 
									? '<img src="'.esc_url($clone_val).'" alt="'.esc_attr__('img', 'organic-beauty').'" />'
			...
			case 'select':
				...
				if (empty($field['style']) || $field['style']=='select') {
					echo '<input class="organic_beauty_options_input organic_beauty_options_input_select" type="text" value="'.esc_attr($caption) . '"'
						...
						. ' />'
						. ($before)
						. '<span class="organic_beauty_options_field_after organic_beauty_options_with_action iconadmin-down-open" onclick="return false;"></span>';
				}
...
if ( !function_exists( 'organic_beauty_options_save' ) ) {
	//Handler of add_action('wp_ajax_organic_beauty_options_save', 'organic_beauty_options_save');
	//Handler of add_action('wp_ajax_nopriv_organic_beauty_options_save', 'organic_beauty_options_save');
	function organic_beauty_options_save() {

		$mode = organic_beauty_get_value_gp('mode');
		$override = empty($_POST['override']) ? 'general' : organic_beauty_get_value_gp('override');
		$slug = empty($_POST['slug']) ? '' : organic_beauty_get_value_gp('slug');
		
		if (!in_array($mode, array('save', 'reset', 'export')) || $override=='customizer')
			return;

		if ( !wp_verify_nonce( organic_beauty_get_value_gp('nonce'), admin_url('admin-ajax.php') ) || !current_user_can('manage_options'))
			wp_die();
		...
		if ($mode == 'save') {
			parse_str(organic_beauty_get_value_gp('data'), $post_data);
		} else if ($mode=='export') {
			parse_str(organic_beauty_get_value_gp('data'), $post_data);
			if (!organic_beauty_storage_empty('post_override_options', 'fields')) {
				$options = organic_beauty_array_merge(organic_beauty_storage_get('options'), organic_beauty_storage_get_array('post_override_options', 'fields'));
			}
		} else
			$post_data = array();
		...
		if ($mode=='export') {
			$name  = organic_beauty_get_value_gp('name');
			$name2 = isset($_POST['name2']) ? organic_beauty_get_value_gp('name2') : '';
			$key = $name=='' ? $name2 : $name;
...
if ( !function_exists( 'organic_beauty_options_import' ) ) {
	//Handler of add_action('wp_ajax_organic_beauty_options_import', 'organic_beauty_options_import');
	//Handler of add_action('wp_ajax_nopriv_organic_beauty_options_import', 'organic_beauty_options_import');
	function organic_beauty_options_import() {

		if ( !wp_verify_nonce( organic_beauty_get_value_gp('nonce'), admin_url('admin-ajax.php') ) || !current_user_can('manage_options'))
			wp_die();
	
		$override = $_POST['override']=='' ? 'general' : organic_beauty_get_value_gp('override');
		$text = stripslashes(trim(chop($_POST['text'])));
		if (!empty($text)) {
			$opt = organic_beauty_unserialize($text);
		} else {
			$key = trim(chop(organic_beauty_get_value_gp('name2')));
			...
			foreach ($post_options as $id=>$field) { 
				...
				if ($mode == 'save' || $mode=='export') {
					if ($override!='general' && organic_beauty_is_inherit_option($post_data[$id.'_inherit']))
						...
						if ($id == 'reviews_marks' && is_array($new) && function_exists('organic_beauty_reviews_theme_setup')) {
							$new = join(',', $new);
