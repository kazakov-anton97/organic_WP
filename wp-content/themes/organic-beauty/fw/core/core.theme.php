...
if ( !function_exists( 'organic_beauty_core_theme_setup' ) ) {
	...
	function organic_beauty_core_theme_setup() {
		...
		if ( !is_admin() ) {
			...
			add_action('wp_footer',		 					'organic_beauty_core_frontend_scripts_inline', 9);
		...
		<div class="logo">
			<a href="<?php echo esc_url(home_url('/')); ?>"><?php
				if (!empty($logo_main)) {
					...
					echo '<img src="'.esc_url($logo_main).'" class="logo_main" alt="'.esc_attr__('img', 'organic-beauty').'"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
				if (!empty($logo_fixed)) {
					$attr = organic_beauty_getimagesize($logo_fixed);
					echo '<img src="'.esc_url($logo_fixed).'" class="logo_fixed" alt="'.esc_attr__('img', 'organic-beauty').'"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
				if (!empty($logo_footer)) {
					$attr = organic_beauty_getimagesize($logo_footer);
					echo '<img src="'.esc_url($logo_footer).'" class="logo_footer" alt="'.esc_attr__('img', 'organic-beauty').'"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
				if (!empty($logo_side)) {
					$attr = organic_beauty_getimagesize($logo_side);
					echo '<img src="'.esc_url($logo_side).'" class="logo_side" alt="'.esc_attr__('img', 'organic-beauty').'"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
...
if ( !function_exists( 'organic_beauty_register_theme_sidebars' ) ) {
    add_action('widgets_init', 'organic_beauty_register_theme_sidebars');
	function organic_beauty_register_theme_sidebars($sidebars=array()) {
        ...
        $registered = organic_beauty_storage_get('registered_sidebars');
        ...
        organic_beauty_storage_set('registered_sidebars', $registered);
	}
}
...
if ( !function_exists( 'organic_beauty_core_frontend_scripts' ) ) {
	function organic_beauty_core_frontend_scripts() {
		...
		wp_enqueue_script( 'modernizr', organic_beauty_get_file_url('js/photostack/modernizr.min.js'), array(), null, false );
	    ...
	    if ( 'off' !== _x( 'on', 'Google fonts: on or off', 'organic-beauty' ) ) {
			...
			if (is_array($theme_fonts) && count($theme_fonts) > 0) {
				...
				foreach ($theme_fonts as $font=>$v) {
					if (isset($fonts[$font])) {
						...
						if (!empty($fonts[$font]['css'])) {
							$css = $fonts[$font]['css'];
							wp_enqueue_style( 'organic-beauty-font-'.str_replace(' ', '-', $font_name).'-style', $css, array(), null );
						} else {
				...
				if ($google_fonts) {
                    $google_fonts_enabled = ( 'off' !== esc_html_x( 'on', 'Google fonts: on or off', 'organic-beauty' ) );
                    if ( $google_fonts_enabled ) {
					    wp_enqueue_style( 'organic-beauty-font-google-fonts-style', add_query_arg( 'family', $google_fonts.'&subset='.organic_beauty_get_theme_option('fonts_subset'), "//fonts.googleapis.com/css" ), array(), null );
                    }
				}
			}
		}
		...
		wp_enqueue_style( 'fontello-style',  organic_beauty_get_file_url('css/fontello/css/fontello.css'),  array(), null);

		// Main stylesheet
		wp_enqueue_style( 'organic-beauty-main-style', get_stylesheet_uri(), array(), null );
		...
		if (organic_beauty_get_theme_option('css_animation')=='yes' && (organic_beauty_get_theme_option('animation_on_mobile')=='yes' || !wp_is_mobile()) && !organic_beauty_vc_is_frontend())
			wp_enqueue_style( 'organic-beauty-animation-style',	organic_beauty_get_file_url('css/core.animation.css'), array(), null );
		...
		if (organic_beauty_get_theme_option('responsive_layouts') == 'yes') {
			wp_enqueue_style( 'organic-beauty-responsive-style', organic_beauty_get_file_url('css/responsive.css'), array(), null );
			do_action('organic_beauty_action_add_responsive');
			$css = apply_filters('organic_beauty_filter_add_responsive_inline', '');
			if (!empty($css)) wp_add_inline_style( 'organic-beauty-responsive-style', $css );
		}
		...
		wp_enqueue_script( 'superfish', organic_beauty_get_file_url('js/superfish.js'), array('jquery'), null, true );

		wp_enqueue_script( 'organic-beauty-core-utils-script',	organic_beauty_get_file_url('js/core.utils.js'), array('jquery'), null, true );
		wp_enqueue_script( 'organic-beauty-core-init-script',	organic_beauty_get_file_url('js/core.init.js'), array('jquery'), null, true );
		wp_enqueue_script( 'organic-beauty-theme-init-script',	organic_beauty_get_file_url('js/theme.init.js'), array('jquery'), null, true );
		...
		if (organic_beauty_get_custom_option('show_video_bg') == 'yes' && organic_beauty_get_custom_option('video_bg_youtube_code') != '') {
			wp_enqueue_script( 'video-bg-script', organic_beauty_get_file_url('js/jquery.tubular.1.0.js'), array('jquery'), null, true );
		}
			
		// Social share buttons
		if (is_singular() && !organic_beauty_storage_get('blog_streampage') && organic_beauty_get_custom_option('show_share')!='hide') {
			wp_enqueue_script( 'social-share-script', organic_beauty_get_file_url('js/social/social-share.js'), array('jquery'), null, true );
		}
		...
		if (organic_beauty_get_theme_option('debug_mode')=='yes') {
			wp_enqueue_script( 'organic-beauty-core-debug-script', organic_beauty_get_file_url('js/core.debug.js'), array(), null, true );
		}
...
if ( !function_exists( 'organic_beauty_enqueue_slider' ) ) {
	function organic_beauty_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			wp_enqueue_style(  'swiperslider-style', 			organic_beauty_get_file_url('js/swiper/swiper.css'), array(), null );
			// jQuery version of Swiper conflict with Revolution Slider!!! Use DOM version
			wp_enqueue_script( 'swiperslider-script', 			organic_beauty_get_file_url('js/swiper/swiper.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'organic_beauty_enqueue_polaroid' ) ) {
	function organic_beauty_enqueue_polaroid() {
		wp_enqueue_style(  'polaroid-style', 	organic_beauty_get_file_url('js/photostack/component.css'), array(), null );
		wp_enqueue_script( 'classie-script',		organic_beauty_get_file_url('js/photostack/classie.js'), array(), null, true );
		wp_enqueue_script( 'polaroid-script',	organic_beauty_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'organic_beauty_enqueue_messages' ) ) {
	function organic_beauty_enqueue_messages() {
		wp_enqueue_style(  'organic-beauty-messages-style',		organic_beauty_get_file_url('js/core.messages/core.messages.css'), array(), null );
		wp_enqueue_script( 'organic-beauty-messages-script',	organic_beauty_get_file_url('js/core.messages/core.messages.js'),  array('jquery'), null, true );
	}
}
...
if ( !function_exists( 'organic_beauty_enqueue_portfolio' ) ) {
	function organic_beauty_enqueue_portfolio($hover='') {
		wp_enqueue_style( 'organic-beauty-portfolio-style',  organic_beauty_get_file_url('css/core.portfolio.css'), array(), null );
		if (organic_beauty_strpos($hover, 'effect_dir')!==false)
			wp_enqueue_script( 'hoverdir', organic_beauty_get_file_url('js/hover/jquery.hoverdir.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'organic_beauty_enqueue_diagram' ) ) {
	function organic_beauty_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') wp_enqueue_script( 'diagram-chart-script',	organic_beauty_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') wp_enqueue_script( 'diagram-raphael-script',	organic_beauty_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if ( !function_exists( 'organic_beauty_enqueue_popup' ) ) {
	function organic_beauty_enqueue_popup($engine='') {
		if ($engine=='pretty' || (empty($engine) && organic_beauty_get_theme_option('popup_engine')=='pretty')) {
			wp_enqueue_style(  'prettyphoto-style',	organic_beauty_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			wp_enqueue_script( 'prettyphoto-script',	organic_beauty_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else if ($engine=='magnific' || (empty($engine) && organic_beauty_get_theme_option('popup_engine')=='magnific')) {
			wp_enqueue_style(  'magnific-style',	organic_beauty_get_file_url('js/magnific/magnific-popup.css'), array(), null );
			wp_enqueue_script( 'magnific-script',organic_beauty_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
...
if ( !function_exists( 'organic_beauty_core_frontend_scripts_inline' ) ) {
	//Handler of add_action('wp_footer', 'organic_beauty_core_frontend_scripts_inline', 1);
...
if ( !function_exists( 'organic_beauty_core_frontend_add_js_vars' ) ) {
	//Handler of add_action('wp_footer', 'organic_beauty_core_frontend_add_js_vars', 2);
	function organic_beauty_core_frontend_add_js_vars() {
		...
		if (!empty($vars)) wp_localize_script( 'organic-beauty-core-init-script', 'ORGANIC_BEAUTY_STORAGE', $vars);
	}
}
...
if (!function_exists('organic_beauty_core_add_property_to_link')) {
	//Handler of add_filter('style_loader_tag', 'organic_beauty_core_add_property_to_link', 10, 3);
...
if (!function_exists('organic_beauty_core_add_scripts_inline')) {
	//Handler of add_action('organic_beauty_action_add_scripts_inline','organic_beauty_core_add_scripts_inline');
...
if (!function_exists('organic_beauty_core_localize_script')) {
	//Handler of add_filter('organic_beauty_filter_localize_script',	'organic_beauty_core_localize_script');
...
if ( !function_exists( 'organic_beauty_add_widget_number' ) ) {
	//Handler of add_filter('dynamic_sidebar_params', 'organic_beauty_add_widget_number', 10, 1);
...
if ( !function_exists( 'organic_beauty_wp_title_show' ) ) {
	// Handler of add_action('wp_head', 'organic_beauty_wp_title_show');
...
if ( !function_exists( 'organic_beauty_wp_title_modify' ) ) {
	// Handler of add_filter( 'wp_title', 'organic_beauty_wp_title_modify', 10, 2 );
...
if ( !function_exists( 'organic_beauty_add_mainmenu_classes' ) ) {
	// Handler of add_filter('wp_nav_menu_objects', 'organic_beauty_add_mainmenu_classes', 10, 2);
...
if ( !function_exists( 'organic_beauty_callback_frontend_editor_save' ) ) {
	function organic_beauty_callback_frontend_editor_save() {
		...
		parse_str(organic_beauty_get_value_gp('data'), $output);
...
if ( !function_exists( 'organic_beauty_callback_frontend_editor_delete' ) ) {
	function organic_beauty_callback_frontend_editor_delete() {
		...
		$post_id = organic_beauty_get_value_gp('post_id');
