...
if (is_admin()) {
	...
	if ( !function_exists( 'organic_beauty_admin_load_scripts' ) ) {
		//Handler of add_action("admin_enqueue_scripts", 'organic_beauty_admin_load_scripts');
		function organic_beauty_admin_load_scripts() {
			wp_enqueue_style( 'organic-beauty-admin-style', organic_beauty_get_file_url('css/core.admin.css'), array(), null );
			if (organic_beauty_check_admin_page('widgets.php')) {
				wp_enqueue_style( 'fontello-style', organic_beauty_get_file_url('css/fontello-admin/css/fontello-admin.css'), array(), null );
				wp_enqueue_style( 'fontello-animations-style', organic_beauty_get_file_url('css/fontello-admin/css/animation.css'), array(), null );
			}

            if (organic_beauty_get_theme_option('debug_mode')=='yes') {
            wp_enqueue_script( 'organic-beauty-debug-script', organic_beauty_get_file_url('js/core.debug.js'), array('jquery'), null, true );
            }
            wp_enqueue_script( 'organic-beauty-admin-script', organic_beauty_get_file_url('js/core.admin.js'), array('jquery'), null, true );
		}
	}
	
	// Prepare required styles and scripts for admin mode
	if ( !function_exists( 'organic_beauty_admin_localize_script' ) ) {
		//Handler of add_filter("organic_beauty_filter_localize_script_admin", 'organic_beauty_admin_localize_script');
		function organic_beauty_admin_localize_script($vars) {
            ...
            $vars['msg_importer_full_alert'] = esc_html__("ATTENTION!\n\nIn this case ALL THE OLD DATA WILL BE ERASED\nand YOU WILL GET A NEW SET OF POSTS, pages and menu items.", 'organic-beauty')
                . "\n\n"
                . esc_html__("It is strongly recommended only for new installations of WordPress\n(without posts, pages and any other data)!", 'organic-beauty')
                . "\n\n"
                . esc_html__("Press 'OK' to continue or 'Cancel' to return to a partial installation", 'organic-beauty');
			return $vars;
		}
	}
	...
	if ( !function_exists( 'organic_beauty_admin_add_js_vars' ) ) {
		//Handler of add_action('admin_footer', 'organic_beauty_admin_add_js_vars', 2);
		function organic_beauty_admin_add_js_vars() {
			...
			if (!empty($vars)) wp_localize_script( 'organic-beauty-admin-script', 'ORGANIC_BEAUTY_STORAGE', $vars);
		}
	}
	
	// AJAX: Get terms for specified post type
	if ( !function_exists( 'organic_beauty_callback_admin_change_post_type' ) ) {
		//Handler of add_action('wp_ajax_organic_beauty_admin_change_post_type', 		'organic_beauty_callback_admin_change_post_type');
		//Handler of add_action('wp_ajax_nopriv_organic_beauty_admin_change_post_type',	'organic_beauty_callback_admin_change_post_type');
		function organic_beauty_callback_admin_change_post_type() {
			if ( !wp_verify_nonce( organic_beauty_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
				wp_die();
			$post_type = organic_beauty_get_value_gp('post_type');
