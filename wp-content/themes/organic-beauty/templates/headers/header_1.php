		...
		<header class="top_panel_wrap top_panel_style_1 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
					...
					<div class="columns_wrap columns_fluid no_margins">
						...
						if (!empty($contact_phone) || !empty($contact_email)) {
							?><div class="column-1_3 contact_field contact_phone">
								<span class="contact_icon"></span>
								<span class="contact_label contact_phone"><a href="tel:<?php organic_beauty_show_layout($contact_phone); ?>"><?php organic_beauty_show_layout($contact_phone); ?></a></span>
								<span class="contact_email"><a href="mailto:<?php organic_beauty_show_layout($contact_email); ?>"><?php organic_beauty_show_layout($contact_email); ?></a></span>
							</div><?php
						}
						...
						if (function_exists('organic_beauty_exists_woocommerce') && organic_beauty_exists_woocommerce() && (organic_beauty_is_woocommerce_page() && organic_beauty_get_custom_option('show_cart')=='shop' || organic_beauty_get_custom_option('show_cart')=='always') && !(is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART'))) {
							?><div class="column-1_3 contact_field contact_cart"><?php do_action('trx_utils_show_contact_info_cart'); ?></div><?php
						}
						?></div>
				</div>
				</div>
			...
			<div class="top_panel_bottom">
				<div class="content_wrap clearfix">
					...
					<?php if (organic_beauty_get_custom_option('show_search')=='yes' && function_exists('organic_beauty_sc_search') ) organic_beauty_show_layout(organic_beauty_sc_search(array("style"=>organic_beauty_get_theme_option('search_style')))); ?>
				</div>
			</div>
