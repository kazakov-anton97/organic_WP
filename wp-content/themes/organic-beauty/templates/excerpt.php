				...
				<?php
					...
					if (!organic_beauty_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
                        if(function_exists('organic_beauty_sc_button')){
                            ...
                            organic_beauty_show_layout('<a href="' . esc_url($post_data['post_link']) . '" class="sc_button sc_button_square sc_button_style_filled sc_button_size_small sc_button_hover_fade">'. $post_options['readmore'] .'</a>');
                        }
					}
				?>
				</div>
