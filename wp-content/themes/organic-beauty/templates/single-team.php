...
if ( !function_exists( 'organic_beauty_template_single_team_output' ) ) {
	function organic_beauty_template_single_team_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && organic_beauty_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="headline" class="post_title entry-title"><?php organic_beauty_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
			<?php 
		}

		if (!$post_data['post_protected'] && (
			!empty($post_options['dedicated']) ||
			(organic_beauty_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])
		)) {
			?>
			<section class="post_featured single_team_post_featured">
				...
				if (is_array($soc_list) && count($soc_list)>0) {
					...
					if (!empty($soc_str)) {
						?><div class="socials_single_team"><?php if(function_exists('organic_beauty_sc_socials')) organic_beauty_show_layout(organic_beauty_sc_socials(array('size'=>"tiny", 'shape'=>'round', 'socials'=>$soc_str))); ?></div><?php
					}
				}
				?>
			</section>
			...
			<section class="single_team_post_description">
				...
				<div class="team_meta"><?php
					if (!empty($post_meta['team_member_email'])) {
						?><p><?php esc_html_e('E-mail: ', 'organic-beauty'); ?><?php organic_beauty_show_layout($post_meta['team_member_email']); ?></p><?php
					}
					if (!empty($post_meta['team_member_bday'])) {
						?><p><?php esc_html_e('Date Of Birth: ', 'organic-beauty'); ?><?php organic_beauty_show_layout($post_meta['team_member_bday']); ?></p><?php
					}
				?></div><?php
				...
				if (!empty($post_meta['team_member_brief_info'])) {
					...
					<div class="team_brief_info">
						<h5 class="team_brief_info_title"><?php esc_html_e('Brief info', 'organic-beauty'); ?></h5>
		...
		if ($show_title && $post_options['location'] != 'center' && organic_beauty_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="name" class="post_title entry-title"><?php organic_beauty_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
			<?php 
		}
