...
if ( !function_exists( 'trx_utils_users_localize_script' ) ) {
	...
	function trx_utils_users_localize_script($vars) {
		...
		$vars['msg_login_failed'] 			= addslashes(esc_html__("Login Failed", 'trx_utils'));
...
if ( !function_exists( 'trx_utils_users_registration_user' ) ) {
	...
	function trx_utils_users_registration_user() {
		...
		$notify = 'both';
		$contact_email = organic_beauty_get_theme_option('contact_email');
		$contact_email = empty($contact_email) ? get_bloginfo('admin_email') : $contact_email;
		...
		if ( is_wp_error($id) ) {
			...
			if (in_array($notify, array('both', 'user', 'yes'))) {
				$subj = sprintf(esc_html__('Welcome to "%s"', 'trx_utils'), get_bloginfo('site_name'));
