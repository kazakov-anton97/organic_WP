		...
		if (organic_beauty_shortcodes_is_used()) {
			...
			organic_beauty_storage_set('sc_params', array(
				...
				'controls'	=> organic_beauty_get_list_controls(),
                    'categories'=> is_admin() && organic_beauty_get_value_gp('action')=='vc_edit_form' && substr(organic_beauty_get_value_gp('tag'), 0, 4)=='trx_' && isset($_POST['params']['post_type']) && $_POST['params']['post_type']!='post'
                        ? organic_beauty_get_list_terms(false, organic_beauty_get_taxonomy_categories_by_post_type($_POST['params']['post_type']))
                        : organic_beauty_get_list_categories(),
